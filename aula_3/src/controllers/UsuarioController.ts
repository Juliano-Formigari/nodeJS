...
export class UsuarioController {
    ...
    static async criaUsuario(req: Request, res: Response, next: NextFunction) {
        try {
            const usuarioExiste = await getRepository(Usuario)
                .createQueryBuilder("usuario")
                .where("usuario.email = :email", { email: req.body.email })
                .getOne();

            if (usuarioExiste) throw createHttpError(404, "Já existe um usuário cadastrado com este e-mail!!")
    ...
    static async atualizaUsuario(req: Request, res: Response, next: NextFunction) {
        try {
            ...
            getRepository(Usuario).merge(usuarioExiste, req.body);
            const resultado = await getRepository(Usuario).save(usuarioExiste);
            res.json(resultado);
